...
function parseSheet(file, player) {
    ...
    return {
        ...
        source: {
            adapter: 'xlsx-campaign-v3', file: basename(file), player,
...
for (const [dir, player] of Object.entries(PLAYER_DIRS)) {
    ...
    for (const f of readdirSync(full).filter((f) => f.endsWith('.xlsx') && !EXCLUDE.test(f))) {
        try {
            ...
            console.log(`✓ ${player.padEnd(6)} ${doc.name}  (${doc.weapons.length} weapons, ${doc.talents.length} talents, ${nSkills} skills, ${doc.aptitudes.length} aptitudes, ${doc.xp.ledger.length} xp entries)`);
        } catch (e) {

(() => {
  ...
  var HIT_LOCATIONS = ["Head", "Right Arm", "Left Arm", "Body", "Right Leg", "Left Leg"];
  ...
  var CHECKPOINTS = Object.freeze({
    ...
    POWER_MODIFIERS: "power.MODIFIERS",
    // before the Focus Power d100 (talents, push bonuses)
    POWER_POST_ROLL: "power.POST_ROLL",
    // roll known; Phenomena decided (flag phenomena / no_phenomena)
    POWER_PHENOMENA: "power.PHENOMENA",
    // after the Phenomena roll (reroll_phenomena, riders)
    POWER_PERILS: "power.PERILS",
    // after a Perils of the Warp roll
    POWER_EFFECT: "power.EFFECT"
    // the power resolves (rider damage, narrative effects)
  });
  ...
  var Parser = class {
    ...
    // --- program / rule ------------------------------------------------------
    parseProgram() {
      const rules = [], tables = [], actions = [], packages = [], specLists2 = [];
      let dslVersion = null;
      while (!this.atEof()) {
        ...
        else if (this.isKw("spec_list")) specLists2.push(this.parseSpecList());
      ...
      return { type: "Program", rules, tables, actions, specLists: specLists2, dslVersion: dslVersion ?? CURRENT_DSL_VERSION, package: packages[0] ?? null, packages };
    }
  ...
  var SLOT_DEFS = {
    ...
    rider_dice: {
      ...
      apply: (ctx, op, v) => {
        ctx.riderDice = (ctx.riderDice || 0) + v;
      }
    },
  ...
  var FLAG_DEFS = {
    ...
    phenomena: {
      ...
      apply: (ctx) => {
        ctx.phenomenaTriggered = true;
        ctx.phenomenaReason = "forced by rule";
      }
    },
    no_phenomena: {
      at: "power.POST_ROLL",
      ...
      apply: (ctx) => {
        ctx.phenomenaTriggered = false;
        ctx.phenomenaCancelledBy = ctx.rollLabel ?? "rule";
      }
    },
    reroll_phenomena: {
      at: "power.PHENOMENA",
  ...
  function compileSpecLists(src, base = []) {
    const program = typeof src === "string" ? parse(src) : src;
    ...
    for (const sl of program.specLists ?? []) {
      ...
      if (!prev) {
        merged.set(key, { name: sl.name, options: [...sl.options], open: !!sl.open });
        continue;
      }
...
# pipeline as "Psychic Powers" (the engine adds 2 \xD7 psy rating for a resisting
...
` };
  ...
  var specListsSrc = readRule("spec-lists.dsl");
  var actionsSrc = readRule("actions.dsl");
  var psychicSrc = readRule("psychic.dsl");
  ...
  var psychicEffects = compile(psychicSrc);
  var rollTables = compileTables(rollTablesSrc);
  var specLists = compileSpecLists(specListsSrc);
  ...
  var availableTalents = referencedNames([talentsSrc, actionsSrc, psychicSrc].join("\n\n")).talents;
  var availableTraits = referencedNames(traitsSrc).traits;
  var availableConditions = referencedNames([conditionsSrc, psychicSrc].join("\n\n")).conditions;
  var availableCircumstances = referencedNames(circumstancesSrc).circumstances;
  var availableConfigurations = referencedNames([configurationsSrc, psychicSrc].join("\n\n")).configurations;
  ...
  var FIRING_MODES = availableConfigurations;
  function buildDefaultRegistry() {
    return new Registry().addAll(combatActionEffects).addAll(qualityConflictEffects).addAll(weaponQualityEffects).addAll(talentEffects).addAll(traitEffects).addAll(conditionEffects).addAll(circumstanceEffects).addAll(configurationEffects).addAll(mechanicEffects).addAll(psychicEffects).addTables(rollTables);
  }
  function buildRegistry(customRules, disabledIds = []) {
    ...
    const registry = new Registry().addAll(combatActionEffects).addAll(qualityConflictEffects).addAll(keep(weaponQualityEffects)).addAll(keep(talentEffects)).addAll(keep(traitEffects)).addAll(keep(conditionEffects)).addAll(keep(circumstanceEffects)).addAll(keep(configurationEffects)).addAll(keep(mechanicEffects)).addAll(keep(actionRuleEffects)).addAll(keep(psychicEffects)).addTables(rollTables);
    if (custom.length) {
  ...
  // api/lib/rules/psychic-strength.mjs
  var PSYKER_CLASSES = Object.freeze({
  ...
  var maxPushFor = (cls) => PSYKER_CLASSES[String(cls ?? "").toLowerCase()]?.maxPush ?? 0;
  var psykerClassList = () => Object.entries(PSYKER_CLASSES).map(([id, c]) => ({ id, ...c }));
  var PSYCHIC_ATTACK_MODES = Object.freeze({
  ...
  function psychicHits(mode, success, dos, effectivePsyRating) {
  ...
  function resolveTest(input, rng = Math.random, registry = defaultRegistry) {
    const ctx = new RollContext({
      ...
      target: input.foe && typeof input.foe === "object" ? { ...input.foe, traits: canonList(input.foe.traits) } : null,
      effects: []
    });
  ...
  var isDoubles = (roll) => roll >= 10 && Math.floor(roll / 10) % 10 === roll % 10;
  function resolveFocusPower(input, rng = Math.random, registry = defaultRegistry) {
    ...
    if (effective !== base) modifiers["psy rating"] = (base - effective) * 10;
    const ctx = new RollContext({
      input,
      characteristics,
      action: "Focus Power",
      testName: "Focus Power",
      isMelee: false,
      rangeBand: "",
      aimValue: 0,
      rng,
      ...
      firingModes: input.configs ?? input.firingModes ?? [],
      combat: { dualWielding: false, firingOffhand: false, firingBoth: false },
      modifiers,
      effects: [],
      target: null,
    ...
    if (power.opposed) {
      ...
      const resist = resolveTest({
        ...
        testName: "Psychic Powers",
        modifiers: oMods,
        unnatural: o.unnatural ?? 0,
        talents: o.talents,
        traits: o.traits,
        conditions: o.conditions,
        circumstances: o.circumstances,
        psyRating: oPr,
        label: "resist (Willpower)"
      }, rng, registry);
      ...
      ctx.opposedWon = won;
      ctx.opposedDos = resist.dos;
    }
    if (power.noPhenomena) {
      ctx.phenomenaTriggered = false;
      ctx.phenomenaReason = "this test cannot generate Psychic Phenomena";
    } else if (push > 0) {
      ctx.phenomenaTriggered = !test.doubles;
      ctx.phenomenaReason = test.doubles ? `pushing +${push} \u2014 doubles (${test.roll}) do NOT trigger Phenomena (p.194)` : `pushing +${push} \u2014 any roll but doubles triggers Psychic Phenomena (p.194)`;
    } else {
      ctx.phenomenaTriggered = test.doubles;
    ...
    if (ctx.phenomenaTriggered) {
      ...
      let result = first;
      ctx.phenomenaRoll = first.roll;
      ctx.phenomenaText = first.text;
      ...
      phenomena = {
        triggered: true,
        reason: ctx.phenomenaReason,
        roll: result.roll,
        natural: result.roll - result.modifier,
      ...
      if (result.roll >= 75) {
        ...
        ctx.perilsRoll = p.roll;
        ctx.perilsText = p.text;
  ...
  var PSYKER_CLASSES2 = ["none", "bound", "unbound", "daemonic"];
  ...
  var CHARACTER_FIELDS = [
    ...
    { path: "psy", type: `{ rating, class: ${PSYKER_CLASSES2.join("|")}, sustained }`, required: false, summary: "Psy rating (0 = not a psyker), psyker class, powers currently sustained." },
  ...
  function validateCharacter(doc, opts = {}) {
    ...
    if (doc.psy !== void 0) {
      if (!doc.psy || typeof doc.psy !== "object") err("psy", "Must be { rating, class, sustained }");
        ...
        if (doc.psy.class !== void 0 && !PSYKER_CLASSES2.includes(doc.psy.class)) err("psy.class", `One of: ${PSYKER_CLASSES2.join(", ")}`);
  ...
  var specListFor = (lists, base) => (lists ?? []).find((l) => norm2(l.name) === norm2(String(base ?? "")));
  function listAvailableAdvances(doc, pack, { includeHeld = false, specLists: specLists2 = [] } = {}) {
    ...
    for (const t of pack.talents) {
      ...
      push({
        ...
        ...t.specialist && specListFor(specLists2, t.name) && {
          specOptions: specListFor(specLists2, t.name).options,
          specOpen: !!specListFor(specLists2, t.name).open
        },
  ...
  function applyOrigin(doc, pack, { homeworldRef, backgroundRef, roleRef, choices = {}, specLists: specLists2 = [] } = {}) {
    var _a;
    const d2 = structuredClone(doc);
    ...
    const resolveGrant = (text, source, kind, applyFn) => {
      ...
      for (const c of points) {
        const list = !c.options ? specListFor(specLists2, c.base) : null;
...
<kind> "<name>" [tier N] {
  ...
  [when <predicate>] then <action> [; <action> ...]
      ...
      notes: [
        ...
        'Pipelines (Phase 3): `on` takes `pipeline.CHECKPOINT`. A bare checkpoint is the default `attack` pipeline; the generic-test pipeline is `test.MODIFIERS` / `test.POST_ROLL` (behind /api/test \u2014 gate on test_name); the Focus Power pipeline is `power.MODIFIERS` / `power.POST_ROLL` / `power.PHENOMENA` / `power.PERILS` / `power.EFFECT` (behind /api/power \u2014 gate on is_power("\u2026"), psyker_class, is_pushing, is_doubles).',
    ...
    checkpoints: [
      ...
      { name: "power.MODIFIERS", group: "Power pipeline", summary: "Before the Focus Power d100 (Phase 6, /api/power). The engine has already applied the \xB110-per-point psy-strength modifier and the power's difficulty; rules add modifiers or bonus rating (`set effective_psy_rating += \u2026`).", use: "Warp Conduit (+1d5 rating when pushing with Fate), situational Focus Power bonuses." },
      { name: "power.POST_ROLL", group: "Power pipeline", summary: "The Focus Power roll is known (roll/success/DoS, is_doubles) and the Phenomena decision is made (doubles, or anything but doubles when pushing). Rules may `flag phenomena` / `flag no_phenomena` and add `set phenomena_roll += \u2026`.", use: "Class modifiers (unbound +10), sustaining, Warp Lock (cancel + 1d5 to the Head)." },
      { name: "power.PHENOMENA", group: "Power pipeline", summary: "After the Table 6\u20132 roll (phenomena_roll). `flag reroll_phenomena` rolls again and keeps the lower; 75+ chains into Perils.", use: "Favoured by the Warp." },
      { name: "power.PERILS", group: "Power pipeline", summary: "After the Table 6\u20133 Perils of the Warp roll (perils_roll).", use: "Narrative riders on a Perils result." },
      { name: "power.EFFECT", group: "Power pipeline", summary: "The power resolves: success/DoS, opposed_won/opposed_dos for opposed powers, effective_psy_rating final. `set rider_dice += \u2026` rolls d10s of direct damage afterwards.", use: "Force weapon rider (+1d10 Energy per DoS on an opposed win, p.145)." }
    ],
  ...
  function mergeActorState(combatant, encounter, key) {
  ...
  function specListsFor(body) {
    ...
    try {
      return compileSpecLists(body.customRules, specLists);
    } catch {
      return specLists;
    }
  }
  var GET = {
    ...
    "/api/options": () => ({
      ...
      // Phase 6: psyker classes (push caps, Table 6–1) + psychic attack modes (p.198)
      psykerClasses: psykerClassList(),
      psychicAttackModes: Object.entries(PSYCHIC_ATTACK_MODES).map(([id, m]) => ({ id, label: m.label, note: m.note }))
    }),
    ...
    "/api/rules": () => ({
      ...
      specLists
      // specialization sets (spec_list declarations)
    }),
  ...
  var POST = {
    ...
    "/api/chargen/advances": (body) => {
      ...
      return {
        advances: listAvailableAdvances(
          doc,
          CHARGEN_PACK,
    ...
    // Phase 6: a Focus Power test through the power.* pipeline (psy strength,
    // doubles → Phenomena → Perils, opposed resist, attack-mode hits, riders).
    "/api/power": (body) => {
      const rng = rollScript(body.forcedRolls ?? []);
      const out = resolveFocusPower(body, rng, registryFor(body));
    ...
    "/api/resolve": (body) => {
      ...
      if (body.encounter || body.attackerKey || body.defenderKey) {
        out.encounter = harvestEngagement(
          ...
          out,
          { attackerName: input.attacker?.weapon?.name ? input.attacker?.name : void 0, defenderName: input.defender?.name }
        );
      }
      out.rollTrace = rng.trace;
      return out;
    ...
    "/api/engage": (body) => {
      ...
      switch (phase) {
        case "attack":
          out = engageAttackRoll(attacker, reg, rng, defender);
          annotateRecharge({ attack: out }, body);
          break;
        case "damage":
          out = engageDamage(attacker, state.attack ?? {}, reg, rng, defender);
          break;
        case "evasion":
          out = engageEvasion(defender, state.attack?.test?.dos ?? 0, reg, rng, state.attack?.preventsParry, attacker.weapon ?? null);
          break;
        case "onhit": {
        ...
        default:
          throw new DslError(`Unknown engagement phase '${phase}'`, 0, 0);
      }
      out.rollTrace = rng.trace;
      return out;
    },
  ...
  function annotateRecharge(result, body) {
    ...
    if (body.encounter?.actors?.[key]?.cooldowns?.recharge && result.attack) {
      ((_a = result.attack).effects ?? (_a.effects = [])).push({
        name: "Recharging",
  ...
  function dispatch(method, path, body = {}) {
    ...
    if (verb === "POST") {
      ...
      if (!fn) return { status: 404, body: { error: `Unknown endpoint ${path}` } };
      try {
        return { status: 200, body: fn(body ?? {}) };
      } catch (err) {
        return { status: 400, body: { error: err.message } };
      }
    }
  ...
  (function installLocalApi() {
    ...
    window.fetch = async (input, init = {}) => {
      ...
      if (!apiPath) return realFetch(input, init);
      const method = (init.method || typeof input === "object" && input.method || "GET").toUpperCase();
      let body = {};
      const raw = init.body ?? (typeof input === "object" ? input.body : void 0);
      if (raw) {
        try {
          body = JSON.parse(raw);
        } catch {

...
npm test           # 266 tests (engine + pipeline + DSL + docs + rules content + character schema/adapters + phase0–3 refactor suites + crit table + weapon data + HTTP endpoints)
...
- `valued-names.test.mjs` — which rules take a severity/level variable (drives the Roll UI value inputs)

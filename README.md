...
npm test           # 350 tests (engine + pipeline + DSL + docs + rules content + character schema v3/adapters + Foundry actor mapper + phase0–5 suites + fire modes + ammo + name normalization + dependencies + campaign roster + crit table + weapon data + HTTP endpoints)

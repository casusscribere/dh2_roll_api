...
npm test           # 296 tests (engine + pipeline + DSL + docs + rules content + character schema/adapters + phase0–5 suites + fire modes + crit table + weapon data + HTTP endpoints)
...
- **Attack actions as first-class fire/attack modes (Phase 5):** the Roll page's action dropdown is now a **weapon-aware radio group** — melee weapons offer Standard/All Out/Charge/Called Shot plus the talent-gated multi-attacks (⚠), ranged weapons offer only the fire modes their RoF supports (Semi-Auto Burst and Suppressing Fire (Semi) need a semi RoF; Full Auto Burst and Suppressing Fire (Full) need a full RoF). New RAW content: **Suppressing Fire** (p.224) in both modes — Hard (−20) BS, per-2-DoS hits capped at the mode's RoF (`hitAccrual` decouples accrual from cap), and the kill-zone Pinning effect (Difficult −10 semi / Hard −20 full) fires **even on a miss**; the **auto-fire 94+ jam** (p.223–224) on bursts and Suppressing Fire (defers to Best craftsmanship and Reliable); **Swift/Lightning Attack corrected to Half Actions** (Table 7-1) with their hit cap fixed to the **WS bonus** instead of the (zero) melee RoF, plus advisory rules for the RAW talent gates and Lightning's Unbalanced/Unwieldy restriction — all authored in `actions.dsl`, whose rules now compile into the registry alongside its action declarations.

...
npm test           # 857 tests (engine + pipeline + DSL + docs + rules content + character schema v3/adapters + Foundry actor mapper + phase0–5 suites + fire modes + ammo + name normalization + dependencies + campaign roster + crit table + weapon data + HTTP endpoints + prose + script coverage)
...
npm run build:engine-lib # bundle the engine as a host-agnostic library → ./dist (vendored by rogue-trader-2e)
```

> **Foundry integration (OD-1, 2026-08-25):** this repo no longer ships or deploys a
> Foundry module. The former `foundry/dh2-roll-vm` walking skeleton and its
> `build:foundry` / `export:packs` / `deploy:foundry` chain are retired (git history
> is the undo). The engine reaches Foundry solely as the **vendored library bundle**
> inside the `rogue-trader-2e` system (`dist/dh2-engine.mjs` → its
> `module/vendor/dh2-engine.mjs`, hash-guarded there against staleness).

Every build writes to a default path inside the repo, and each of those roots can
be redirected with an environment variable (the same convention as `CORPUS_DIR`)
— useful for building into a scratch directory, and what the
script coverage suite uses to keep real builds off the working tree:

| var | overrides | default |
|---|---|---|
| `DH2_DOCS_DIR` | `build:static` output, `serve:static` document root | `docs/` |
| `DH2_DIST_DIR` | `build:engine-lib` output | `dist/` |
| `DH2_CHARGEN_DIR` | `sync-chargen` output | `api/data/chargen/` |
| `CORPUS_DIR` | corpus `sync-chargen` reads | `../../codified-systems/dark_heresy_2e` |

Open http://localhost:3210 for the UI. It has six pages: **Home** (navigation + status), **Power** (the Focus Power roller — see *Psychic powers (Phase 6)* below), **Builder** (character advancement + creation: the CB-3 panel spends XP on aptitude-costed advances straight from the rulebook tables with a full audited ledger (every entry source-noted) and undo, a **GM override door** that adds any talent/trait/skill rank/psy rating with prerequisites ignored at 0 XP — source-marked `manual override` — and the CB-4 creation flow — **integrated into the sheet since 2026-08-27**: the Origin / Characteristics / Wounds-&-Fate / Divination blocks live in the main panel, appear in sequence for a new Acolyte, stay visible AND editable throughout (loaded characters show their creation choices too), and changing an earlier step re-derives everything after it, replaying purchases at current prices with conflicts surfaced; the build validator now also reports creation findings (unselected origin, unresolved choice points, RAW range violations, fate below threshold, Psyker/Untouchable clashes) — walks origin → characteristics (RAW 2d10+25 with home-world keep-high/low dice and one reroll, or manual entry — method recorded in `extensions.builder.wizard`) → wounds/fate incl. the Emperor's Blessing roll → divination → starting-XP spend (elite advances included — buying one applies its **instant changes** as 0-XP ledger grants sourced to the EA, and unlocks its gated talents) → Equip Acolyte (kit-class guidance + the RAW Influence-bonus acquisition allowance) → validation; every step is applied by the engine through the same dispatch routes, so the headless acceptance suite `builder-flow.test.mjs` drives exactly what the page does), **Roll** (d100 test, raw damage, Parry, and a **Full Attack Resolution — Engagement** checklist with attacker / weapon / actions / defender zones, per-side talent toggles, Conditions/Circumstances/Configurations inputs, Evasion/Field reactions, on-hit target tests, and report controls — a **verbose** detail toggle, a **debug** toggle for per-roll die-forcing + reroll, and pause/skip pacing), **Characters** (the campaign roster as a read-only **character sheet modeled on the Foundry acolyte sheet** — its tab set Main/Combat/Gear/Advances/Psychic/Bio and its panels: characteristics table with unnatural + bonus, wounds/fate pools, per-location armour, force field, talent/trait chips, weapon cards; v2-pending sections shown dimmed with their `source.unmapped` notes; deep-links `characters.html?c=<id>` and a "→ load on Roll page" button that pre-selects the preset via `roll.html?preset=<id>`), and **Rules** (click any built-in rule to toggle it on/off; add, edit, validate, toggle, and remove your own DSL rules; an active/inactive summary; the full active DSL; and the DSL reference). Rule toggles and custom rules are stored per-browser (localStorage); rolls apply only the rules toggled active. **Hover cards (ST-2)**: talent/trait/skill chips and Builder advance rows show the rulebook paragraph on hover in local builds (the git-ignored prose overlay), and degrade to citation-only ("Core Rulebook p.128 — text available in local builds") on the public Pages site, per decision D-N.
...
| GET | `/api/options` | — | actions, range bands, aim modes, hit locations, psyker classes (push caps), psychic attack modes |
...
| POST | `/api/power` | `{characteristics:{wp}, target?, power:{name, difficulty, opposed?, noPhenomena?, attackMode?}, psyRating, effectivePsyRating?, psykerClass?, sustained?, modifiers?, unnatural?, talents?, traits?, conditions?, configs?, opposed?:{characteristics:{wp}, psyRating?, talents?…}, forcedRolls?}` | a **Focus Power** test through the `power.*` pipeline (Phase 6): `psy {base, effective, push, class, cap, final}`, `test` (+`doubles`), `opposed {test, won, margin}`, `phenomena {triggered, reason, roll, modifier, text, statuses, rolls}`, `perils`, `hits` (attack modes), `rider` (Force), `declaredDamage`, `effects`, `rollTrace` |
...
| GET | `/api/characters` | — | the campaign preset roster (id, name + full character document each; human player names are stripped at import) — generated by `npm run import:campaign` |
| GET | `/api/chargen/pack` | — | the prose-stripped chargen data pack (aptitudes, cost tables, homeworlds/backgrounds/roles, talent/skill/trait catalogs; every entry carries `citation {book, page, source}`) — generated by `npm run sync:chargen` |
| GET | `/api/prose` | — | `{available, count, prose}` — the sourcebook-text overlay keyed by `dh2:<type>:<id>`, each `{text, sha256, citation}`. **`available:false` unless the git-ignored overlay is present** (see *Sourcebook text* below) |
...
- **Psychic powers (Phase 6):** the `power.*` pipeline (`power.MODIFIERS` → `POST_ROLL` → `PHENOMENA` → `PERILS` → `EFFECT`) behind **`POST /api/power`** and the new **Power** page, all RAW-cited to Chapter VI. **Psychic strength** (p.194): choose an effective psy rating at or below the base rating (+10 per point under) or **push** above it (−10 per point), capped per class by Table 6–1 (bound +2, unbound +4, daemonic +3 — [api/lib/rules/psychic-strength.mjs](api/lib/rules/psychic-strength.mjs)); sustaining ≥2 powers lowers the final rating by the count (p.198). **Focus Power test** on Willpower (or an explicit target for Perception/Psyniscience powers) with the power's difficulty; **doubles** (tens = ones; 100 counts) trigger **Psychic Phenomena**, and pushing inverts it (anything BUT doubles). **Table 6–2 Psychic Phenomena** (26 rows) and **Table 6–3 Perils of the Warp** (18 rows, chained on 75+) are `roll_table`s in [roll-tables.dsl](api/data/rules/roll-tables.dsl), verbatim from pp.196–197 with the conditions they impose. The class and sustaining modifiers to the Phenomena roll, the Warp-tainted condition (+10/+10, p.197) and the psychic talents are DSL content in the new [psychic.dsl](api/data/rules/psychic.dsl): **Favoured by the Warp** (roll twice, lower kept, both reported; never on a Perils result), **Warp Conduit** (configuration "Warp Conduit (Fate)" while pushing: +1d5 final rating — potency only, the push penalty stays on the chosen rating — and +30 to Phenomena), **Warp Lock** (configuration "Warp Lock": Phenomena ignored, 1d5 Energy to the Head), **Bastion of Iron Will** (+5 × psy rating) and **Strong Minded** (re-roll advisory) on the resist side. **Opposed powers** (p.195): the resister's Willpower test runs through the `test.*` pipeline as **"Psychic Powers"** (so Resistance (Psychic Powers) fires) with **double the psy rating** of a resisting psyker; the psyker wins only by passing AND out-degreeing the resister. **Psychic Bolt / Barrage / Storm / Blast** (p.198) hit counts via `power.attackMode` (1 / 1 + ⌊(DoS−1)/2⌋ / DoS, capped at the effective rating / area). **Force weapon rider** (p.145): manifest the power "Force Weapon" (opposed, no Phenomena, base rating) — on a win the DSL rule in weapon-qualities.dsl declares `rider_dice += dos` and the engine rolls +1d10 Energy per DoS ignoring Armour and Toughness. New vocabulary: facts `effective_psy_rating`, `push`, `is_pushing`, `psyker_class`, `sustained`, `power_name`, `is_doubles`, `phenomena`, `phenomena_roll`, `perils_roll`, `opposed_won`, `opposed_dos`; function `is_power("…")`; slots `effective_psy_rating`, `phenomena_roll`, `perils_roll`, `rider_dice`; flags `phenomena`, `no_phenomena`, `reroll_phenomena`. `resolveTest` now exposes `psy_rating` (the resister side). The **Power page** loads a roster/custom character (WP, psy rating, class, sustaining, talents, known powers), picks the power/difficulty/opposed/attack mode and the effective rating (with the modifier previewed), and prints the whole chain — test, opposed result, Phenomena (both rolls when Favoured), Perils, hits, rider damage — with an optional forced-dice field.
- **Attack-loop content sweep (Phase 5):** the last four DH2 weapon qualities and an attack-relevant talent/trait tranche, all RAW-cited. **Force** (p.145, static half): +psy rating damage & pen and Energy type in a psyker's hands (`psyRating` input / `psy_rating`/`is_psyker` facts; the Focus Power rider is Phase 6). **Indirect** (p.147): −10 to hit, every hit lands `1d10 − BSB` metres off (`declare scatter_hit`, clamped ≥ 0, Scatter Diagram direction), misses scatter X d10. **Smoke** (p.149): `declare smoke <radius> duration 1d10+10` on hit or at the miss scatter point — composes with Blast (one scatter, both payloads). **Spray** (p.149): no attack roll (auto-hit, always Body, Called Shot ignored), targets roll Agility with the generic **`avoids_hit`** flavor of `require_test` (a pass voids the hit's wounds), natural 9 on a damage die jams. **Talents** (talents.dsl): Marksman cancels the Long/Extreme range penalty, Precision Killer removes the Called-Shot −20 (specialised or bare entries), Mighty Shot/Crushing Blow add `half(bs_bonus)`/`half(ws_bonus)` damage, Two-Weapon Master erases the dual-wield penalty, Hatred (+10 WS vs a "Hated Foe"-flagged engagement), Iron Jaw & Die Hard as upkeep policy (Die Hard `suppress`es the Blood Loss Fatigue and rolls Willpower instead). **Traits** (traits.dsl): Auto-Stabilised cancels the Unbraced configuration (−30, p.219 — a per-shot stance toggle), Fear rolls its rating penalty in the `test.*` pipeline against the new `foe` input on `/api/test`, From Beyond surfaces its immunity, Sturdy +20 vs Grapple/Knock Down/Takedown, Regeneration ticks a Toughness test each TURN_START. Encounter actors now carry `stats.talents`/`stats.traits` so upkeep rules can gate on them; the Rules page gained a **search/filter bar** (text, category, active/inactive/replaced state) over the built-in rule browser.
...
- **Configurations reorganised:** **Unbraced** moved from Circumstances to **Configurations** (a per-shot stance the player chooses, not an environment), and the old Off-Hand entry became the **DualWield (main hand)** / **DualWield (off-hand)** pair (p.228): off-hand alone = a single weapon in the off hand (−20, Ambidextrous cancels); main hand = fighting with two weapons (Two-Weapon Wielder −20 keys off it, with a RAW advisory when the talent is missing); both = the off-hand half of a dual-wield turn. The dual_wielding/firing_offhand facts read the configs OR the legacy combat flags, and the Roll page Combat-state checkboxes send the configuration names.
- **Character-sheet input mode + item rules (browser edits):** the Characters page gains an **✎ input mode** — advances steppers and a **manual ±100 modifier** (stored as a distinct `source: "manual"` entry, never merged with derived sources) on every characteristic and skill/speciality, a **movement modifier** (an AgB-delta for the brackets only), insanity/corruption steppers, and **talent/trait toggles** (disabled entries are skipped when the preset loads onto the Roll page). The **equipped/stored badge is now a click-toggle** — encumbrance, derived armour, and item effects recalculate live. Edits persist per-browser (**CharStore**, ui/char-store.js) as an overlay on the server roster (↺ reset any time; the Roll page's preset dropdown marks edited characters ✎ and always applies the edited document). Vitals add **Size, Psy Rating, and the movement brackets**; the armour box became **Defenses** (per-location AP + TB + soak, force field folded in). Weapon cards gain a **→ Roll button** (deep-links roll.html?preset=<id>&weapon=<i> and auto-applies). An **Add-item editor** (name/subtype/quality/weight/description) accepts optional **item-granted DSL rules**, engine-validated and applied at roll time **only while the item is equipped** — with the new **`is_test("Skill")`** DSL function for spelling-blind "+X to <skill> [when <condition>]" modifiers (e.g. a Good Auspex granting +20 Tech-Use).
- **Reverse adapter (Task 7.1, Phase 4/E-3):** the Foundry actor mapper is **bidirectional** — `foundryActorToCharacter` inverts the exported mapping tables and round-trips the entire campaign roster losslessly (`normalizeForRoundTrip` defines the canonical comparison form; engine-only fields ride `flags['dh2-roll-vm']`). The old in-Foundry export command (7.1.4) is moot: the module lane is retired (OD-1) — an export surface belongs to `rogue-trader-2e` via the vendored bundle.
- **Character↔Roll plumbing (Delta 2) + Foundry importer v3:** loading a preset now offers a **weapon picker** (every PC carries several weapons — name/damage/clip per option) instead of silently taking the first; the attacker panel gains a **Psy Rating** input auto-filled from the document (and threaded into the engagement, so Force weapons work from the UI); and the d100 box gains a **Skill picker** listing the applied attacker's skills and specialities with RAW `skillTarget` targets — picking one sets the test target AND the test name, so `test.*` rules (Sturdy, Resistance…) gate correctly. The **Foundry importer is v3**: a pure mapper ([api/lib/foundry-actor.mjs](api/lib/foundry-actor.mjs), headlessly tested against the DH3 template shapes) turns a character document into the full acolyte Actor — characteristics with advances + summed **modifiers-by-source** (per-source breakdown preserved in module flags), camelCase **skills with specialist specialities**, xp → experience, tarot → `bio.divination`, fatigue with the derived threshold, and embedded Items for weapons (clip/equipped/weight), gear, aptitudes, talents, traits, psychic powers (loadout flag), disorders/malignancies/mutations, critical injuries, and the force field. `game.dh2vm.importCharacter(<roster doc>)` in Foundry now yields a fully-populated Actor.
...
- **Pack export v1 (Phase 3 / Lane C)** *(retired 2026-08-25 with the module lane — OD-1)***:** `npm run export:packs` generated Foundry compendia **from the DSL source** — `roll_table` declarations → native **RollTable** documents, weapon-quality rules → **`attackSpecial` Items** (with `hasLevel` from the valued-names analysis and package · book · page provenance in the description) — written as source JSON under `foundry/dh2-roll-vm/packs-src/` and compiled to **v12+ LevelDB** via `@foundryvtt/foundryvtt-cli` (no legacy NeDB), with deterministic name-hashed ids for stable diffs. `deploy:foundry` ships them; the join smoke asserts both packs load and Corrosive carries its p.145 cite.
- **Foundry walking skeleton (Phase 1 / Lane C)** *(retired 2026-08-25 — OD-1: `dark-heresy-3rd-edition` became reference-only and the module lane was dropped; the engine now reaches Foundry only as the library bundle vendored by `rogue-trader-2e`)*: `npm run build:foundry` bundled the whole VM as a Foundry esmodule into `foundry/dh2-roll-vm/`; `npm run deploy:foundry` built + copied it into the live install. Dual-target: `dark-heresy-3rd-edition` (Foundry v14, primary) + `dark-heresy-2nd` (v12, compat). Exposed `game.dh2vm` (VM + `importCharacter` + `mapActor`) and the `/dh2attack` chat command, with a Playwright join smoke. Findings recorded in [FOUNDRY_MIGRATION.md](FOUNDRY_MIGRATION.md).
...
dh2_roll_api/
  ...
  ui/                  ← UI: static front-end (no build step)
    index.html  roll.html  power.html  characters.html  builder.html  rules.html
    style.css  rules-store.js  char-store.js  prose-tip.mjs  builder-core.mjs
...
It also **swaps `api/lib/prose.mjs` for a cite-only stub**, so the sourcebook-text overlay
can never be inlined into the published bundle (see *Sourcebook text* below).
...
## Sourcebook text (ST-1, decision D-N)

Verbatim rulebook text is copyrighted expression; page citations are facts. This repo is
**public** and publishes `docs/` to Pages, so the two are split:

| Surface | Gets |
|---|---|
| This repo, `api/data/chargen/pack.mjs`, the published Pages build | **Citations only** — every pack entry carries `citation { book, page, source }`, e.g. `{ book: 'Core Rulebook', page: 120, source: 'src_dh2_core_p121' }` |
...
`api/lib/prose.mjs` exposes the runtime seam — `loadProseOverlay()` (async, memoised) plus
the pure `bookLabel()` / `citationOf()` helpers re-exported from `api/lib/prose-citation.mjs`.
Set `DH2_PROSE_OVERLAY` to an absolute path to point the loader elsewhere (tests do).
...
> ActiveEffect mirror → distributable module; Lane C retired 2026-08-25 per OD-1 —
> the engine ships to Foundry only as the `rogue-trader-2e` vendored bundle now).
...
- `api/lib/rules/psychic-strength.mjs` — Table 6–1 push caps per psyker class + Psychic Bolt/Barrage/Storm/Blast hit counts (native rule data for the power.* pipeline)

### `ui/` — frontend
- `ui/` — multi-page UI: `index.html` (home/nav), `roll.html` (roller), `power.html` (Focus Power roller, Phase 6), `characters.html`, `builder.html`, `rules.html` (rule manager), `style.css` (shared), `rules-store.js` (localStorage rule store + server validation, shared by the roll & rules pages)
...
- `phase6.test.mjs` — the power.* pipeline: psy strength ±10/pt and class push caps, doubles/push Phenomena triggers (100 = doubles), Phenomena → Perils chain with row conditions, class/sustaining/Warp-tainted modifiers, Favoured by the Warp / Warp Conduit / Warp Lock, opposed resist (double PR, tie ≠ win, Resistance/Bastion/Strong Minded through test.*), the Force rider dice, attack-mode hits, custom power.* rules, `/api/power` + `/api/options` + combatant fields

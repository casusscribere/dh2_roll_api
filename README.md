...
npm test           # 351 tests (engine + pipeline + DSL + docs + rules content + character schema v3/adapters + Foundry actor mapper + phase0–5 suites + fire modes + ammo + name normalization + dependencies + campaign roster + crit table + weapon data + HTTP endpoints)
...
and `FOUNDRY_DATA`) — useful for building into a scratch directory, and what the
...
| `DH2_MODULE_DIR` | `build:foundry` + `export:packs` output, and the dir `deploy:foundry` copies FROM | `foundry/dh2-roll-vm/` |
| `DH2_CHARGEN_DIR` | `sync-chargen` output | `api/data/chargen/` |
| `CORPUS_DIR` | corpus `sync-chargen` reads | `../../codified-systems/dark_heresy_2e` |
| `FOUNDRY_DATA` | Foundry data dir `deploy:foundry` copies INTO | local FoundryVTT path |
